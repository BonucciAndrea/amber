...
place where a default lives and one place to extend. The other verbs are presets over it:

| verb | draws |
|---|---|
| `chart d` | anything below, fully specified |
| `plot v` · `plot (v;W;H)` | one line series (both original shapes still work) |
| `plots (a;b;c)` · ``plots `a`b!(x;y)`` | several series, with a legend |
| `xyplot (xs;ys)` | y against a **real x axis**, so a curve may double back |
| `scatter (xs;ys)` | points, unjoined |
| `step v` · `area v` | piecewise-constant; filled to the zero line |
| `hist v` · `hist (v;nbins)` | a binned distribution |
| ``barh `a`b!3 7`` | labelled horizontal block bars, for categories |
| ``heat `a`b!(r1;r2)`` | a matrix as truecolour cells |
...
**More points than pixels are drawn as an envelope.** Past roughly `4W` points, each pixel
...
the unframed canvas, which is what `examples/graphs.k` builds its hand-drawn rasters on. Its
scratch planes moved from a fixed 12,000-byte stack buffer to the arena.
