...
#     h-1  box bottom   ╰────────────────────────────────╯
#      h   info line    ⬡ amber <version> · exec · mem · [native|portable] …
...
# The banner carries the engine version, and src/a.h is the single place it is
# defined (see the "release identity" block there).  Reading it from the header
# rather than hard-coding it means a release bump no longer fails this suite.
def _amber_version():
    src = open(os.path.join(ROOT, "src", "a.h"), encoding="utf-8", errors="replace").read()
    parts = []
    for k in ("MAJOR", "MINOR", "PATCH"):
        m = re.search(r"#define AMBER_VERSION_%s\s+(\d+)" % k, src)
        parts.append(m.group(1) if m else "?")
    return ".".join(parts)

BRAND = "⬡ amber " + _amber_version()
ESC = b"\x1b"
RELEASE = ESC + b"[r"
...
check(BRAND.encode() in txt,                        "truecolor hex logo + brand on the info line")
...
try:
    ...
    check(BRAND in disp[23], "[pyte] info line on the last row")
    ...
    pump(0.2)
    check(BRAND in screen.display[23], "[pyte] wheel-down restores the normal info line")
